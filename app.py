...
def start_bot_process(bot):
    if bot.id in ACTIVE_PROCESSES:
        try:
            ...
            pass
    try:
        BOT_LOGS[bot.id] = [f"[{datetime.utcnow().strftime('%H:%M:%S')}] INITIALIZING BOT PROCESS: {bot.name}"]
...
<head>
    ...
    <style>
        ...
        .glass-panel { background: rgba(12, 6, 6, 0.92); backdrop-filter: blur(25px); border: 1px solid rgba(220, 38, 38, 0.25); }
        ...
        .logo-glow { filter: drop-shadow(0 0 10px rgba(239, 68, 68, 0.6)); }
    </style>
</head>
        ...
        <aside class="w-72 bg-[#070303] border-r border-red-950/60 flex flex-col justify-between hidden lg:flex">
            <div>
                ...
                <div class="flex items-center space-x-3.5 px-6 h-24 border-b border-red-950/60">
                    ...
                    <div>
                        <span class="text-lg font-black tracking-wider uppercase text-white block leading-tight">VX HOSTING</span>
                        <span class="text-[9px] text-red-500 font-extrabold tracking-widest uppercase">ELITE CLOUD PLATFORM</span>
                    </div>
                </div>

                <nav class="p-4 space-y-2">
                    <button @click="activeTab = 'dashboard'" :class="activeTab === 'dashboard' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'text-slate-400 hover:bg-red-950/30'" class="w-full flex items-center space-x-3.5 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all">
                        <i class="fa-solid fa-chart-pie w-5"></i><span>Dashboard</span>
                    </button>
                    <button @click="activeTab = 'deploy'" :class="activeTab === 'deploy' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'text-slate-400 hover:bg-red-950/30'" class="w-full flex items-center space-x-3.5 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all">
                        <i class="fa-solid fa-cloud-arrow-up w-5"></i><span>Deploy Bot</span>
                    </button>
                    <button @click="activeTab = 'billing'" :class="activeTab === 'billing' ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'text-slate-400 hover:bg-red-950/30'" class="w-full flex items-center space-x-3.5 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all">
            ...
            <div class="p-6 border-t border-red-950/60">
                <div class="bg-black/80 border border-red-900/30 p-4 rounded-2xl">
                    <div class="flex items-center space-x-3">
                        ...
                        <span class="text-xs font-black text-slate-200">SYSTEM ONLINE</span>
                    </div>
                    <p class="text-[10px] text-slate-400 mt-1">Uptime SLA: 99.99% Guaranteed</p>
                </div>
            </div>
        </aside>
            ...
            <header class="h-24 border-b border-red-950/60 bg-[#070303]/90 flex items-center justify-between px-6 lg:px-10 backdrop-blur-md sticky top-0 z-50">
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 class="text-base font-black uppercase text-white tracking-wide" x-text="activeTab"></h1>
                            <span class="text-[10px] text-red-500 font-extrabold tracking-widest block">VX CLUSTER v2.4</span>
                        </div>
                    </div>
                </div>

                <div class="flex items-center space-x-4">
                    <button @click="activeTab = 'billing'" class="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider glow-red transition-all flex items-center space-x-2">
            ...
            <main class="p-6 lg:p-10 max-w-7xl mx-auto w-full space-y-8">
                ...
                <div x-show="activeTab === 'dashboard'" class="space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="glass-panel p-6 rounded-3xl glow-red">
                            ...
                            <span class="text-[10px] text-slate-400 mt-1 block">Running 24/7 Cloud Process</span>
                        </div>
                        <div class="glass-panel p-6 rounded-3xl">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Active Tier</p>
                            <h3 class="text-2xl font-black mt-2 text-emerald-400">Enterprise Elite</h3>
                            <span class="text-[10px] text-slate-400 mt-1 block">Razorpay Live Integrated</span>
                        </div>
                        <div class="glass-panel p-6 rounded-3xl">
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Cluster Storage</p>
                            <h3 class="text-2xl font-black mt-2 text-white">NVMe High Speed</h3>
                            <span class="text-[10px] text-slate-400 mt-1 block">Ultra Low Latency Node</span>
                        </div>
                    </div>
                    ...
                    <div class="glass-panel rounded-3xl overflow-hidden p-6">
                        <div class="flex items-center justify-between pb-6 border-b border-red-950/60">
                            <h3 class="text-lg font-black uppercase text-white flex items-center space-x-2">
                                <i class="fa-solid fa-server text-red-500"></i>
                                <span>Deployed Bots</span>
                            </h3>
                                ...
                                <tbody class="divide-y divide-red-950/40 text-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-4 px-4 text-right space-x-2">
                                            <button @click="fetchLogs({{ bot.id }})" class="px-3.5 py-2 bg-black border border-red-950 rounded-xl text-xs text-slate-300 hover:border-red-500 transition-all"><i class="fa-solid fa-terminal mr-1.5"></i> Terminal</button>
                                    ...
                                    {% else %}
                                    <tr>
                                        <td colspan="4" class="py-8 text-center text-slate-500 font-semibold">No active bots deployed. Click "Deploy Bot" to start hosting your script 24/7.</td>
                                    </tr>
                                    {% endfor %}
                                </tbody>
                            </table>
                    ...
                    <div x-show="selectedBotLog !== null" x-cloak class="glass-panel rounded-3xl p-6 space-y-4">
                        <div class="flex items-center justify-between">
                            <h4 class="font-black text-white text-base flex items-center space-x-2">
                                ...
                                <span>Subprocess Console Logs</span>
                            </h4>
                            <button @click="selectedBotLog = null" class="text-slate-400 hover:text-white"><i class="fa-solid fa-xmark text-lg"></i></button>
                        </div>
                ...
                <div x-show="activeTab === 'deploy'" class="max-w-xl mx-auto">
                    <div class="glass-panel rounded-3xl p-8 glow-red">
                        <div class="text-center space-y-2 mb-6">
                            ...
                            <h2 class="text-2xl font-black text-white uppercase">Deploy Python Script</h2>
                            <p class="text-xs text-slate-400">Upload your Telegram bot script (.py or .zip) to run 24/7.</p>
                        </div>
                        <form action="{{ url_for('upload_bot') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                            <div>
                                ...
                                <input type="text" name="bot_name" required placeholder="e.g., MyTelegramBot" class="w-full bg-black/80 border border-red-950 rounded-2xl px-4 py-3.5 text-white text-sm focus:border-red-500 focus:outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-black uppercase text-slate-400 mb-2">Choose Hosting Tier</label>
                            ...
                            <div>
                                <label class="block text-xs font-black uppercase text-slate-400 mb-2">Bot Script File (.py or .zip)</label>
                            ...
                            <button type="submit" class="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-black py-4 rounded-2xl uppercase tracking-wider text-sm glow-red transition-all">Start Bot Instance</button>
                        </form>
                    </div>
                </div>
                ...
                <div x-show="activeTab === 'billing'" class="space-y-8">
                    <div class="text-center max-w-2xl mx-auto space-y-3">
                        ...
                        <h2 class="text-3xl font-black text-white uppercase">Choose Your Plan</h2>
                        <p class="text-sm text-slate-400">Upgrade your hosting server with dedicated CPU cycles and 24/7 uptime.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4">
                        ...
                        <div class="glass-panel rounded-3xl p-8 flex flex-col justify-between space-y-6">
                            <div class="space-y-4">
                                ...
                                <ul class="space-y-3 text-xs text-slate-300">
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> 1 Active Bot</li>
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> 24/7 Uptime</li>
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> Live Console Logs</li>
                                </ul>
                            </div>
                        ...
                        <div class="glass-panel rounded-3xl p-8 flex flex-col justify-between space-y-6 glow-red border-red-500/50 relative overflow-hidden">
                            ...
                            <div class="space-y-4">
                                ...
                                <ul class="space-y-3 text-xs text-slate-300">
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> Up to 5 Active Bots</li>
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> Priority CPU Power</li>
                        ...
                        <div class="glass-panel rounded-3xl p-8 flex flex-col justify-between space-y-6">
                            <div class="space-y-4">
                                ...
                                <ul class="space-y-3 text-xs text-slate-300">
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> Unlimited Bots</li>
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> Dedicated RAM Node</li>
                                    <li><i class="fa-solid fa-check text-red-500 mr-2"></i> VIP Fast Support</li>
                                </ul>
                            </div>
                            <button @click="payWithRazorpay('Ultimate Cluster', 49900)" class="w-full bg-black border border-red-900/60 hover:bg-red-600 text-white font-black py-3.5 rounded-2xl text-xs uppercase tracking-wider transition-all">Pay ₹499 Now</button>
                        </div>
                    </div>
                </div>
            </main>
...
def create_order():
    ...
    try:
        ...
        new_tx = Transaction(order_id=order_id, plan_name=plan_name, amount=amount, status="Created")
...
def payment_success():
    ...
    if tx:
        ...
        tx.status = "Success"
        db.session.commit()
        flash(f'Upgraded to {plan_name} successfully via Razorpay!', 'success')
...
def upload_bot():
    ...
    bot_token = token_match.group(0) if token_match else "no_token_found"
    ...
    flash(f'Bot "{bot_name}" deployed and started successfully!', 'success')
    return redirect(url_for('index'))

@app.route('/bot/<int:bot_id>/logs')
def get_bot_logs(bot_id):
    return jsonify({"logs": BOT_LOGS.get(bot_id, ["Console initialized. Waiting for output..."])})
...
def control_bot(bot_id, action):
    ...
    if action == 'delete':
        ...
        db.session.commit()
        flash('Bot instance deleted.', 'success')
    return redirect(url_for('index'))
